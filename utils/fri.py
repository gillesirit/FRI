...
def hamming_numbers(dim,i,set_of_pairs):  #compute for each hamming dist h, the number of (a,b) s.t. H(a,b)=h
...
def Dif_weight_hamming_distance(dim,i,set_of_pairs,max_distance):
    ...
    for (a,b) in set_of_pairs:         
        dag_list = Dag(a[0:dim],b[0:dim])
        hamming = len(dag_list)
        if (i in dag_list) and (hamming <= max_distance): #lazy evaluation
            ag_att+=1
            if a[dim]!=b[dim]:
                 m_att+=1/hamming  # DIFFERENCE WITH SURYANI          
...
def fri(dimension,i,sample_set,set_of_pairs,max_distance):  #return FRI(i) for feature i
